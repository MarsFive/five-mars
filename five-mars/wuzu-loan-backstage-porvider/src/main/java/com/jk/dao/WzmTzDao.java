...
import com.jk.util.TzCustomerRelationProvider;
...
@Mapper
public interface WzmTzDao {
    //投资
	@SelectProvider(type = TzCustomerRelationProvider.class, method = "listByTzAttr")
	List<Management> queryTzPage(@Param("management") Management management, @Param("start") int start, @Param("rows") int rows);

	@SelectProvider(type = TzCustomerRelationProvider.class, method = "countByTzAttr")
	Long queryTzTotal(@Param("management") Management management);
	...
	//转出
	@Select(" select t.*,y.investortypename,m.name,b.buyertypename from t_management t ,t_investortype y,t_member m,t_buyertype b where y.id=t.investortypeid and b.id=t.buyerstypeid and m.id=t.buyersid\n" +
			"\n" +
			"\t\t\t\n" +
	...
	@Select(" select * from t_management where 1=1  ")
